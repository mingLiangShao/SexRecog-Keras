...
model = load_model('/Users/shaomingliang/Downloads/BossSensor-master/store/sex.h5')
...
while True:    
    ...
    facerect = cascade.detectMultiScale(img, scaleFactor=1.2, minNeighbors=3, minSize=(50, 50))
    if len(facerect) <= 0:
    ...
    for rect in facerect:
        ...
        if result == 1:  # boss
            rect = facerect[0]
            cv2.putText(frame,"male", (x,y), cv2.FONT_HERSHEY_SIMPLEX, 2, 255, 3)
            cv2.rectangle(frame, tuple(rect[0:2]), tuple(rect[0:2] + rect[2:4]), color, thickness=2)
            cv2.imshow("capture", frame)
            print('Boss is approaching')
        else:
            rect = facerect[0]
            cv2.putText(frame,"female", (x,y), cv2.FONT_HERSHEY_SIMPLEX, 2, 255, 3)
